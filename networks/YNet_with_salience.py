# -*- coding: utf-8 -*-
"""
Implementation of the Y-Net network with Salience Generator for multimodal image segmentation tasks.
This network combines the Y-Net architecture with a salience generator to highlight important regions.
...
class YNet_WithSalience(nn.Module):
    """
       1) Class Definition and Initialization
       2) Dual Downsampling Paths: two separate downsampling paths are defined for different modalities.
       3) Fusion Layers: combine features from both modalities at different levels.
       4) Center Block: processes the fused features from the deepest level.
       5) Upsampling Path: gradually restore the spatial dimensions through upsampling or transposed convolutions.
       6) Salience Generators: generate salience maps to highlight important regions in the input data for each modality.
       7) Forward Propagation:
             Features are extracted from both modalities through their respective downsampling paths, processed by the salience generators, fused at different levels, and then upsampled to produce the final segmentation result.

    """
    def __init__(self, in_channels, n_classes, feature_scale=4, is_deconv=True, is_batchnorm=True):
        super(YNet_WithSalience, self).__init__()
        ...
        # Salience generator for each modality
        self.salience_m1 = Salience(1, 1)
        self.salience_m2 = Salience(1, 1)

    def forward(self, inputs_m1, inputs_m2):
        # Salience generation for each modality
        salience_map_m1 = self.salience_m1(inputs_m1)
        inputs_m1 = inputs_m1 + inputs_m1 * salience_map_m1

        salience_map_m2 = self.salience_m2(inputs_m2)
        inputs_m2 = inputs_m2 + inputs_m2 * salience_map_m2
...
class UnetUp2(nn.Module):
    """
       class for upsampling, combine features from the fusion layers and the center block
    """
