...
class UNet_3D_withSalience(nn.Module):
    def __init__(self, in_channels, n_classes, feature_scale=4, is_deconv=True, is_batchnorm=True):
        ...
        # downsampling
        self.conv1 = UnetConv3(in_channels, filters[0], self.is_batchnorm, kernel_size=(
        ...
        self.wavelet = Salience(1, 1)

    def forward(self, inputs):
        
        # Salience generation
        x_1 = self.wavelet(inputs.transpose(4, 2)).transpose(4, 2)
        x_2 = self.wavelet(inputs.transpose(4, 3)).transpose(4, 3)
        x_3 = self.wavelet(inputs)
        # Integration
        inputs = inputs + inputs * torch.cat([x_1, x_2, x_3], dim=1)
        ...
        center_1 = self.center(maxpool4)

        # extract the features
        self.featuremap_center = center_1.detach()

        center = self.dropout1(center_1)
