# -*- coding: utf-8 -*-
"""
Implementation of the TransUNet network with Salience Generator for image segmentation tasks.
This network combines the TransUNet architecture with a salience generator to highlight important regions.
...
class TransUNet_WithSalience(nn.Module):
    """
       1) Class Definition and Initialization
       2) Downsampling Path: Multiple conv blocks extract features at different levels.
       3) Transformer Encoder: 
             The _create_transformer method creates a Transformer encoder using TransformerEncoderLayer.
             The Transformer processes the features to capture long-range dependencies.
       4) Upsampling Path: gradually restore the spatial dimensions through upsampling or transposed convolutions.
       5) Salience Generator: generates a salience map to highlight important regions in the input data.
       6) Forward Propagation:
           Features are extracted through the downsampling path, processed by the salience generator and the Transformer, and then upsampled to produce the final segmentation result.
    """
    def __init__(self, in_channels, n_classes, img_dim, feature_scale=4, is_deconv=True, is_batchnorm=True):
        super(TransUNet_WithSalience, self).__init__()
...
class UnetUp2(nn.Module):
    """ 
       combine features from the downsampling path and the Transformer output
    """
...
class UnetConv2(nn.Module):
    """
       classes for convolution extract features at different levels
    """
