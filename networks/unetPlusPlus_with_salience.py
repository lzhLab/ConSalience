# -*- coding: utf-8 -*-
"""
Implementation of the U-Net++ network with Salience Generator for image segmentation tasks.
This network combines the U-Net++ architecture with a salience generator to highlight important regions.
...
class UNetPlusPlus_WithSalience(nn.Module):
    """
       1) Class Definition and Initialization
       2) Downsampling Path:
             Multiple conv layers extract features at different levels.
             Max pooling layers reduce the spatial dimensions of the feature maps.
       3) Center Block:
             The center block further processes the features from the deepest level.
       4) Upsampling Path with Nested Concatenation:
             Multiple upsampling blocks combine features from different levels.
             Restore the spatial dimensions through upsampling or transposed convolutions.
       5) Final Convolution Layers:
             Final convolution layers map the features
       6) Salience Generator:
             Generate a salience map to highlight important regions in the input data.
    """
    def __init__(self, in_channels, n_classes, feature_scale=4, is_deconv=True, is_batchnorm=True):
        super(UNetPlusPlus_WithSalience, self).__init__()
...
class UnetConv2(nn.Module):
    """
       Classes for convolution operations
    """
